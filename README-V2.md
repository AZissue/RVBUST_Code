...
新增统一 AI Provider 管理：管理员进入 `/settings/ai` 配置服务商、默认模型及功能分配。快速工单优先 AI、失败自动降级；未配置 Key 时继续使用规则。首次部署执行 `npm run ai:init-key` 后重启 API。密钥保护、接口位置、测试边界见 [AI集成说明.md](AI集成说明.md)。
