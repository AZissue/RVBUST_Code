# 更新日志

本文件记录 MultiCameraCalibration 每个版本的缺陷修复、功能新增与优化。

版本号规则（语义化版本）：
- `v1.0.x`：小修复 / 文案调整 / bugfix / 日志与文档更新
- `v1.x.0`：中等规模（功能优化、模块重构、新增子功能）
- `vx.0.0`：大型功能 / 架构升级（全新模块、重大重构、里程碑发布）

运行时版本号会在基础版本后自动附加 Git 短 hash，例如 `v1.0.4+g<hash>`。

---

## [1.6.1] - 2026-09-09

### Fixed
- 修复模式 C（转台 360° 拼接）「打开会话」时
  `读取转台会话元数据失败: name 'json' is not defined`：
  `backend_bridge.py` 缺少模块级 `import json`，导致 `turntable_meta.json`
  标定数据无法恢复，只能恢复步进帧、必须重新标定。
  现在打开会话会同时恢复转台标定（角度/旋转轴/旋转中心/步数），
  步进采集直接开放，清空拍摄数据后即可更换工件重新采集拼接。

---

## [1.6.0] - 2026-09-04

### Added
- 模式 A（多相机外参标定）会话升级：
  - 「保存会话」现在同时保存标定结果（reference_id + 全部 pair 变换），
    外参不再依赖单独导出；
  - 「打开会话」自动恢复标定帧、扫描帧与标定结果：质量门禁/结果表格
    直接回填，进入扫描阶段后可立即对恢复的扫描帧执行拼接；
  - 标定帧与扫描帧带 kind 标记落盘（`FrameData.kind`，
    兼容无 kind 的旧会话：按 frame_id 首组判定为标定帧）。

### Fixed
- 修复「打开会话」未把点云载入扫描流程导致拼接失败的问题：
  扫描帧现在正确恢复到 `frames_scan`，可直接「拼接并保存」；
- 修复加载会话后仍需重新检测标记物才能标定的问题：标记随会话
  落盘恢复，无标定结果时可直接「计算外参」；
- 修复「加载外参」误选会话 meta.json 显示质量门禁 100% 但拼接失败
  （位姿图无 pair 标定）的问题：`CalibrationEngine.load_calibration`
  现在对缺失/空 pairs、无效 T 矩阵严格报错并给出引导提示；
- 修复加载会话后单相机卡片「2D 预览 / 3D 拍摄 / 检测」按钮被禁用
  的问题：相机在线时自动恢复可用；
- `FixedMultiCamWorkflow.can_proceed` 在已标定状态进入扫描前
  增加 pair 结果与质量校验。

### Changed
- 弃用标定 Tab 中单独的「保存外参 / 加载外参」入口
  （外参随会话保存/恢复，权限更高）；信号与后端处理保留未移除；
- `CalibrationEngine` 新增 `to_dict()` / `load_from_dict()`，
  `OfflineSession` 新增 `set_meta()` / `get_meta()`。

---

## [1.5.0] - 2026-09-02

### Added
- 模式 C（转台 360° 拼接）支持会话保存/恢复标定数据：
  - `TurntableCalibrator` 新增 `set_calibration()`，可直接用保存的
    旋转轴/旋转中心/角度/步数恢复标定；
  - `OnlineTurntableSession` 新增 `apply_calibration()`；
  - 无步进帧时也可保存会话（仅标定元数据）；
  - 加载会话后自动恢复标定状态，「步进采集」直接开放，
    无需重新拍摄 frame0/frame1 标定；
  - 若会话中包含步进采集帧，会一并恢复到 3D 查看器，可直接拼接。

### Changed
- `src/ui_v2/backend_bridge.py`：模式 C 会话加载分支从“不支持”改为
  恢复标定 + 恢复序列帧。

---

## [1.4.0] - 2026-09-02

### Added
- 转台 360° 拼接工作区新增「清空拍摄数据」按钮：
  - 清空当前步进采集的序列帧、点云与拼接结果；
  - 保留标定结果（旋转轴/角度/步数），便于更换工件后在同一标定基础上重新拍摄、拼接并保存不同的合并点云；
  - 清空前弹出确认框，防止误删已采集数据。

### Changed
- 修正步进采集的步数计数逻辑：
  - `current_step` 现在表示已采集的序列帧数（0 → 总步数），不再多 +1；
  - 采集到最后一帧后停止自动递增，并弹出「采集完成」提示；
  - 采集完成后「拍摄当前步 / 开始自动采集」按钮自动禁用，避免超拍。

---

## [1.3.5] - 2026-09-02

### Fixed
- 修复 `src/ui_v2/workspaces/turntable_workspace.py` 中 `_update_online_ui` 的
  `TypeError: setEnabled(list)` 错误：
  `self.session.get_all_frames()` 返回列表，与布尔值做 `or` 运算后仍可能为列表，
  现用 `bool(...)` 包裹，保证传入 `setEnabled` 的是布尔值。

---

## [1.3.4] - 2026-09-02

### Fixed
- 修复转台 360° 拼接工作区拍摄 frame0 后 UI 未更新、再次拍摄卡住的问题：
  - `src/ui_v2/workspaces/turntable_workspace.py` 在拍摄前释放旧帧 RVC 资源，
    拍摄完成后释放当前帧 RVC 资源，避免 SDK 缓冲/句柄被占满导致后续 `Capture()` 阻塞。
  - 新增 `FrameData.release_rvc()`，仅释放 RVC PointMap/Image，保留 `image_np` 与 markers。
  - 将 `_run_busy_worker` 的 `_busy` 释放时机改为 `on_done` 执行完毕后，
    避免结果处理期间用户误触再次触发拍摄。
  - 为 `_update_online_ui` 与 `_done` 增加异常捕获与日志输出，便于后续定位。

---

## [1.3.3] - 2026-09-02

### Fixed
- 修复转台 360° 拼接工作区在拍摄 frame0（未检测到标记物）后，
  点击「2D 预览」会卡住、相机不再响应的问题：
  - 根因：M 系列线扫相机在执行 3D `Capture()` 后，SDK 的 `Capture2D()`
    调用会阻塞，导致预览工作线程一直等待。
  - 修复：`src/core/camera_manager.py` 的 `SingleCameraController.capture_2d`
    检测到线扫模式（`line_scan_detected=True`）且为 X1 相机时，
    改用 3D `Capture()` 取图并丢弃点云，避免阻塞。

---

## [1.3.2] - 2026-09-02

### Fixed
- 修复 `src/ui/viewer_3d.py` 中 `QMatrix4x4.lookAt` 调用签名错误：
  使用 `m.lookAt(eye, center, up)` 形式，解决点云无法渲染、
  `TypeError: lookAt expected 3 arguments, got 9` 异常刷屏的问题。
- 修复真实相机连接失败后仍可进入主工作区的问题：
  `src/ui_v2/backend_bridge.py` 现在会判断所有真实相机均打开失败时阻止进入工作区，
  并弹出提示框列出常见原因（IP 未配置、相机被 RVCManager/其他进程占用、网络异常）
  及排查/解决方法。
- 修复点击「参数调试」后主程序 UI 未正确刷新为未连接状态的问题：
  `src/ui_v2/main_window.py` 在拉起 RVCManager 前主动断开当前相机，
  关闭调试窗口后界面与按钮状态正确显示「未连接相机」。

---

## [1.3.1] - 2026-08-31

### Added
- 新增后处理测试原型 `prototypes/postprocess_test/`：
  - 左侧 DB 树参考 CloudCompare 风格，支持按文件层级加载 PLY/PCD/XYZ；
  - 中间复用 `src/ui_v2/widgets/viewer_panel.py` 的 3D 点云查看器；
  - 右侧后处理面板：体素下采样、统计离群点去除、AABB/球/OBB 裁切；
  - 底部日志栏（LogPanel）实时输出加载/处理/保存信息；
  - 3D 查看器设置 `MAX_RENDER_POINTS = 100,000,000`，显示原始点云，
    不触发显示级体素下采样。
- 后处理测试原型 UI 采用 ui_v2 风格：应用 `theme.GLOBAL_QSS`，
  使用与主程序一致的图标、按钮与面板样式。

### Fixed
- 修复后处理测试原型 `prototypes/postprocess_test/app.py` 的模块导入路径：
  将 `sys.path` 指向 `src/` 目录，而非项目根目录，解决 `ModuleNotFoundError: No module named 'core'`。

---

## [1.3.0] - 2026-08-31

### Added
- 新增后处理测试原型 `prototypes/postprocess_test/`：
  - 左侧 DB 树参考 CloudCompare 风格，支持按文件层级加载 PLY/PCD/XYZ；
  - 中间完整 3D 点云查看器，复用 `src/ui/viewer_3d.py`；
  - 右侧后处理面板：体素下采样、统计离群点去除、AABB/球/OBB 裁切；
  - 支持将后处理结果保存为新点云文件。

---

## [1.2.6] - 2026-08-31

### Fixed
- 修复多相机外参标定工作区同步/异步单选按钮文本被遮挡的问题：
  ModernRadioButton 增加 sizeHint / minimumSizeHint，根据文本长度自动计算所需宽度。

---

## [1.2.5] - 2026-08-31

### Changed
- 主窗口顶部工具栏合并「设备管理」与「模式」按钮：
  删除重复的「模式：xxx ▾」按钮，保留「设备管理」入口，
  当前模式仍通过窗口标题、底部状态栏显示。

---

## [1.2.4] - 2026-08-31

### Changed
- 多相机外参标定工作区「拍摄控制」同步/异步单选按钮样式现代化：
  改为与启动小窗设备表格一致的圆角矩形 + 品牌红背景 + 白色对勾，
  去掉原生单选圆点和焦点虚线框。

---

## [1.2.3] - 2026-08-31

### Fixed
- 恢复 Test 相机进入主功能页面的能力，用于 UI 调试：
  - 模式 A（多相机外参标定）：≥2 台设备即可进入（真实或 Test 均可）；
  - 模式 B（单相机移动拼接）：≥1 台设备即可进入；
  - 模式 C（转台 360° 拼接）：≥1 台设备即可进入。
- 当真实相机不足但测试相机补齐时，弹窗提示「调试模式」，
  明确告知测试相机仅用于 UI 布局调试，拍摄/标定/拼接等真实数据功能不可用。
- 真实相机连接失败时仍保留带排查建议的弹窗（关闭 RVCManager、检查网络等）。

---

## [1.2.2] - 2026-08-31

### Fixed
- 修复单相机移动拼接模式（模式 B）允许仅通过 Test 相机进入工作区的 bug：
  现在三种模式（A/B/C）均要求至少 1 台真实相机成功连接，
  仅选择测试相机时会弹窗提示「测试相机仅用于布局预览」。

---

## [1.2.1] - 2026-08-31

### Fixed
- 修复多相机外参标定 / 转台 360° 拼接连接 Test 相机或真实相机失败时，
  启动小窗关闭后没有明确错误提示的问题。
- `BackendBridge` 现在会根据连接失败原因自动分类：
  - 仅选择测试相机：提示测试相机仅用于布局预览；
  - 真实相机被占用：提示关闭 RVCManager / 结束占用进程；
  - 真实相机数量不足：提示模式所需的最低相机数量；
  - 所有真实相机连接失败：给出网络/IP/物理连接等排查建议。
- `MainWindow` 在连接失败回退弹窗中追加排查建议，
  特别针对 RVCManager 占用相机的场景给出操作指引。

---

## [1.2.0] - 2026-08-31

### Added
- 主窗口顶部工具栏新增「参数调试」按钮（位于「打开会话」之后）。
  点击后：先回到设备连接页面（LauncherDialog），断开当前已连接相机，
  然后自动查找并打开 RVC 官方调试工具 `RVCManager.exe`。
  默认查找路径：
  - `D:\Program Files\RVBUST\RVC\RVCManager\RVCManager.exe`
  - `C:\Program Files\RVBUST\RVC\RVCManager\RVCManager.exe`
  若均未找到，弹窗提示并允许用户手动指定 `RVCManager.exe` 路径。

### Changed
- `ui_v2/icons.py` 新增「sliders」线性图标，用于「参数调试」按钮。

---

## [1.1.3] - 2026-08-31

### Changed
- 更新 `README.md`：修正当前版本号示例、删除已移除的 `web-ui-shell/` 目录、
  统一架构图与开发计划中机器人手眼模块的模式编号为 E（原错写为 D）。
- 更新 `docs/README.md`：新增第六轮代码审查文档索引，修正模式 D/E 说明，
  同步整理日期。

---

## [1.1.2] - 2026-08-31

### Fixed
- **P2-c3** `station_timeline.update_station()` 布局插入索引错误，
  改为按 `layout_index` 原位替换，避免删除/重排站位后界面布局错乱。
- **P3 N12** 多相机检测改为串行执行并加 `_detect_lock` 保护，
  消除 `MarkerDetector` 共享状态在多线程并发下的竞态条件。
- **P3 N13** 转台 360° 拼接增加 `_busy` 门控，
  2D 预览、frame0/frame1 拍摄、步进采集互斥，防止操作重叠导致崩溃。

### Changed
- **P3 N9** 主窗口初始大小与居中改用 `QApplication.screenAt(QCursor.pos())`，
  多显示器环境下不再默认挤到主屏幕左上角。
- **P3 N5** 新增 `ruff.toml` 静态检查配置，统一代码风格。
- **P3 N6** `version._parse_version()` 兼容 `v` 前缀与非数字后缀，
  解析失败时回退 `(0,0,0)` 避免启动崩溃。
- **P3 N11** 转台保存会话成功后立即 `dirty_changed.emit(False)`，
  不再保留已保存的脏标记。
- **P3 N4** `README.md` 统一模式命名为 D（机器人手眼），修正全局 BA 说明。
- **P3 N14** 删除未使用的 `web-ui-shell/` 并加入 `.gitignore`；
  清理 `.git/objects/pack/tmp_pack_*` 临时文件。
- **P3 N1/N2** 创建 git tag `v1.1.1` 并规范化版本管理。

### Tests
- `test_core.py`、`test_chain_stitcher.py`、`test_robot_stitch.py`、
  `test_integration.py`、`test_ui.py` 全部通过。

---

## [1.1.1] - 2026-08-31

### Fixed
- **P2-c1** `ChainStitcher.get_merged_pointcloud()` 返回内部缓存的深拷贝，
  防止调用方修改后污染缓存。
- **P2-c2** 新增 `pcd_utils.merge_pointclouds()`，合并前统一 colors/normals 属性，
  修复 `o3d.PointCloud +=` 在属性不对称时颜色/法向量被清零的问题；
  已应用到 `chain_stitcher`、`stitch_engine`、`robot_stitch_workflow` 三处。
- **P2-c10** `TurntableCalibrator.generate_sequence()` 每帧先复制 base_pcd 再 transform，
  避免原地修改导致所有帧变成同一姿态；同步修复原型侧相同 bug。

---

## [1.1.0] - 2026-08-31

### Fixed
- **N2** 主窗口重新打开设备管理后连接设备时显示 loading 遮罩；
  若连接失败，弹窗提示并允许回退到之前的模式。
- **N4** 转台 360° 自动采集改为当前帧完成后再调度下一拍，
  避免采集慢于间隔时 worker 堆积、帧乱序。
- **P2-2** `mobile_chain_workflow.load_session_dir` 加载失败时回滚到加载前状态，
  不再留下半初始化的链式拼接器。

### Changed
- **N5** 离线拼接原型 `simple_ui.py` 拼接循环内增加 `QApplication.processEvents()`，
  loading 遮罩现在能正确显示实时进度。

---

## [1.0.9] - 2026-08-31

### Fixed
- **N1** 主窗口关闭时选择保存会话后同步等待保存完成，避免异步保存被中断导致文件半写。
- **N3** 修正模式 B/C 打开离线会话后的日志文案：
  不再无条件显示"会话加载完成"，模式 B 提示时间线恢复待实现，模式 C 提示暂不支持会话恢复。
- **P2-u2** 单相机卡片 3D 拍摄失败后恢复该卡片的 2D 预览勾选状态。
- **P2-u3** 实时取景面板 `set_frame(None)` 现在会清空画布，不再保留最后一帧。
- **P2-u8** 多相机外参标定模式要求至少 2 台相机成功连接才置 connected；
  转台 360° 拼接模式要求至少 1 台相机成功连接才置 connected。

---

## [1.0.8] - 2026-08-31

### Fixed
- **P0** 修复单相机移动拼接离线加载会话后删除/撤销/失败帧会删除整个会话目录的数据丢失问题：
  `load_session_dir` 中每帧 `offline_dir` 改为站位子目录，与在线拍摄路径一致。
- **P1** 修复 `load_session_dir` 按字符串排序导致 `station_10` 排在 `station_2` 前面的问题，
  改为按站位序号数字排序，避免 ≥10 机位会话丢站/参考系错位。
- **P1** 同步 `prototypes/turntable_360_stitch/core/turntable_calibrator.py` 的转台点云复制修复，
  避免原地 transform 导致多次拼接结果漂移。
- **P2** 修复 `src/core/turntable_calibrator.py` 拼接消息构造时未过滤 `None` 帧的潜在崩溃。
- 转台模式拼接成功后也置脏标记，关闭窗口时触发未保存确认。

### Changed
- 复选框选中态增加白色对勾 SVG，提升可访问性。

---

## [1.0.7] - 2026-08-28

### Changed
- 进一步优化多相机外参标定工作区同步/异步拍摄单选按钮：
  指示器改为正圆形，选中态显示品牌红圆环 + 白点，与复选框视觉明显区分。

---

## [1.0.6] - 2026-08-28

### Changed
- 多相机外参标定工作区「已连接设备」列表选中样式现代化：
  移除焦点虚线框，选中项改为左侧品牌红强调边 + 半透明选中底色。
- 同步/异步拍摄单选按钮样式现代化：自定义 16px 圆角指示器，
  选中态显示品牌红实心圆环，悬停/禁用态统一视觉层级。

---

## [1.0.5] - 2026-08-28

### Fixed
- 修复转台 360° 拼接模式进入时崩溃：
  `TurntableWorkspace.set_camera_manager()` 存在两个重复定义，
  导致 `main_window.set_mode()` 调用参数不匹配；
  移除旧的两步注入方法，统一使用合并后的 `set_camera_manager(cam_mgr, marker_detector)`。
- 修复转台模式连接相机遮罩卡住的问题，现在可正常进入主功能页面。

---

## [1.0.4] - 2026-08-28

### Changed
- 转台 360° 拼接工作区移除独立的「初始化 RVC / 刷新 / 连接相机 / 断开」按钮，
  改为直接使用 LauncherDialog 已连接的共享相机管理器。
- 转台 360° 拼接 2D 预览区改为带标题栏的卡片式面板，UI 更现代。

---

## [1.0.3] - 2026-08-28

### Changed
- 暂时移除主窗口顶部「后处理」入口按钮与 `_PostProcessDialog`；
  点云预览窗口为保证运行性能默认执行的显示级下采样仍然保留；
  后处理功能将在后续版本中通过独立设置/预览页面重新提供。
- 更新 README.md 与 docs/README.md，同步模式说明、开发计划与 CHANGELOG 索引。

---

## [1.0.2] - 2026-08-28

### Fixed
- 单相机移动拼接模式 B 进入时自动创建空会话目录的问题：现在会话目录延迟到首次拍摄机位时才创建。
- 关闭主窗口时无数据也会提示保存的问题：现在仅当存在未保存的拍摄数据时才弹窗，并提供「保存 / 不保存 / 取消」三选一。

### Added
- 单相机移动拼接工作区新增「离线加载会话」按钮，可选择已保存的 `session_*` 文件夹，自动识别 `station_N/station_N.png + station_N.ply` 并按顺序拼接。

---

## [1.0.1] - 2026-08-27

### Changed
- 版本号移除 `alpha` 后缀，进入正式语义化版本管理。
- `version.get_version()` 运行时自动附加当前 Git commit 短 hash。
- 程序启动时在日志中输出当前版本号，便于排查问题。
- 新增 `CHANGELOG.md`，集中记录版本变更与缺陷修复。

---

## [1.0.0] - 2026-08-27

首个正式版本，包含以下主要功能与缺陷修复：

### Added
- **多相机外参标定模式**：支持多相机同步/异步拍摄、编码圆检测、外参计算与保存。
- **单相机链式拼接模式**：单相机移动拍摄，多帧按编码圆配准并拼接。
- **转台 360° 拼接模式**：作为第三种独立模式并入主程序，支持转台场景点云拼接。
- **编码圆标定板生成器**：
  - 支持扇区数 N、同心圆半径比、整体缩放等参数实时调节；
  - 预览图与导出 PDF/PNG 严格一致；
  - 支持 A4 页面、打印 DPI、页面边距设置。
- **离线拼接测试工具（ui_v2 重构版）**：
  - 递归/模糊匹配本地图像与点云文件对；
  - 离线识别编码圆并支持加载 `coded_circle_meta.json`；
  - 拼接结果缓存，文件列表顶部新增【合并结果】入口，切换后不丢失；
  - 合并点云保存与显示分离，保存仍写完整分辨率。
- **3D 点云查看器**：
  - N 路相机点云叠加显示；
  - 合并结果独立显示模式；
  - 最大化、折叠、视角预设、坐标轴、网格、背景切换等工具栏；
  - 大数据量自动降采样渲染，避免 UI 卡死。

### Fixed
- **严重 bug**：从启动小窗切换模式后，主界面固定显示「多相机外参标定」且错误显示仅 1 台相机的问题。
- 自动配置 IP 时未显示 loading、UI 卡住的问题。
- 3D 查看器最大化/全屏按钮在多模式与离线工具中失效的问题。
- 转台模式进入后先跳到多相机拼接再闪回正确 UI 的问题。
- 主程序启动小窗完成连接后仍闪一下的问题。
- 主功能初始窗口超出屏幕分辨率、控件显示不全的问题。
- 编码圆生成器保存 PDF 时因源文件被占用导致失败的错误提示不清晰的问题。
- `BackendBridge` 中缺失 `Signal` 导入导致的启动报错。
- 离线拼接读取数据文件对失败、合并结果未缓存导致切换后消失的问题。
- 离线拼接 2600 万点合并结果渲染极慢的问题（显示级下采样 + 快速粗降路径）。

### Optimized
- 启动流程改为先渲染目标工作区 + loading，再后台连接相机，避免空白页面与闪烁。
- 主程序窗口启动时默认屏幕居中。
- 文本/标题边框统一去掉，蓝色框标题更醒目。
- 工具栏图标与按钮边缘清晰度优化。
- 日志窗口折叠按钮与图标样式优化。

---

## 历史预发布版本

### [1.0.0-alpha] - 早期验证阶段
- P1 算法验证版本，双模式工作流架构原型。
- 包含多相机标定与单相机链式拼接基础能力。
