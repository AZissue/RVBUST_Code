...
- 当前状态：v2.0 阶段 3/4/7/8 联测反馈第 2 轮已全部收口。①读取按钮并入主界面底部
  ActionButtons 同一行、样式字体与预览/拍照/计算一致；按钮文案去掉「读取」改为
  「拍照位姿」「戳点位姿」，改用紧凑内边距（新增 secondaryButtonCompactStyle，
  padding 0 12px、最小宽 88）避免窄窗口字体截断；拍照时自动读取勾选在工具面板
  机器人通信页。②3D 复位按钮改为悬浮原生无边框胶囊、重叠于场景左下角，实测通过。
  机器人真机未到位，通信链路暂留接口待真机实测。AICode 与 D 工作区构建+单测全绿；
  阶段 5/6/9/10 未开工。
- 下一步：阶段 8 其余 3 项（姿态引导/数据质检/板位姿可视化）规格确认；机器人/Modbus
  真机到位后再实测通信链路。
- 正在进行的文件：无（本轮已收口）。
- 卡点 / 风险：机器人/Modbus 真机未到位（通信链路暂未实测，仅留接口）；
  阶段 8 姿态引导/质检/板位姿可视化规格未确认；阶段 4 外参语义待真实线扫数据验证。
...
- [ ] 阶段 8 其余 3 项（姿态引导/数据质检/板位姿可视化）规格确认 → 实现
