...
- 下会话待办（2026-08-26 用户测试反馈，**仅记录未实现**，逐条见「下一步任务」）：
  ① load_cloud「文件路径 / 批量文件夹」改为二选一互斥（设置其一则另一禁用）；
  ② 属性面板「全选」后 3D 视窗反而清空（bug，需修复）；
  ③ 新增「平移/旋转」工具：点选某一帧可独立平移/旋转，其它帧不动、刷新后保持；
     ROI「框选」按钮改为 Box ROI 节点专属（移入节点功能按钮区）；
  ④ Box ROI 参数重构：去掉 frame_filter（有 ROI 节点必然每帧处理）；盒数量用 +/−
     按钮增减（至少 1）；单帧多盒、每盒可独立操控、可设 ID；盒列表从节点参数改为
     点云对象属性（每帧自己的盒列表），方案待评估（效率/代码）后定；
  ⑤ 选中中间节点「运行到当前」后 3D 视窗不刷新该节点输出（需再点一次才显示）；
     「显示输出」下拉改为「显示数据类型」多选筛选（点云/包围盒/直线…，默认全勾，
     后续几何类型扩展）；
  ⑥ 运行交互：未选中节点时「运行到当前」按钮禁用并提示；运行时运行按钮有明显
     交互提示（现左下角不明显）；运行中不禁用 3D 视窗操作，但禁止画布/参数操作
     （连线、删节点、改参与运行节点参数），违规操作日志提示；
  ⑦ 节点连线命中区域加大（当前不易选中）；
  ⑧ 画布缩小到一定程度切到另一种布局后，放大无法切回，且该布局不能放置节点
     （修复缩放回切 + 布局只读）。
...
| 属性面板「全选」后 3D 视窗清空 | 待修 | 2026-08-26 用户反馈（下会话） |
| 选中节点「运行到当前」后视窗不刷新该节点输出 | 待修 | 2026-08-26 用户反馈（下会话） |
| 节点连线不易选中 | 待修 | 命中区域加大（下会话） |
| 画布缩小切换布局后放大无法切回、且不能放节点 | 待修 | 缩放回切 + 只读布局（下会话） |
...
- [ ] load_cloud 文件路径 / 批量文件夹二选一互斥
- [ ] 属性面板全选 → 3D 显示全部选中帧（修复清空 bug）
- [ ] 「平移/旋转」工具：点选单帧独立变换、刷新后保持；ROI 框选移入 Box ROI 节点按钮区
- [ ] Box ROI 重构：去 frame_filter；盒数量 +/− 按钮；单帧多盒可操控可设 ID；盒列表改点云对象属性
- [ ] 运行到当前后视窗即时刷新选中节点输出
- [ ] 「显示输出」改为「显示数据类型」多选筛选（点云/包围盒/几何…默认全勾）
- [ ] 运行交互：未选中节点禁用「运行到当前」+ 提示；运行按钮忙碌提示；运行中禁画布/参数操作并日志提示
- [ ] 节点连线命中区域加大
- [ ] 画布缩放布局回切修复（缩小切大纲、放大切回画布；大纲布局只读）
