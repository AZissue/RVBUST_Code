...
- 当前状态：v2.0 阶段 3/4/7/8 联测反馈的 4 项 UI 修正已完成（①机器人通信面板
  迁到工具面板 + 主界面按连接态显示「读取拍照位姿/读取戳点位姿/拍照时自动读取」+
  模拟连接成功按钮；②窗口默认居中、标题栏不再顶出屏幕；③窗口边缘缩放去卡顿；
  ④2D/3D 视窗去标题栏（标题改视窗内浮层）+ 3D 复位工具栏透明无边框）。
  AICode 与 D 工作区构建+单测全绿，待用户按 UI 清单实测；阶段 5/6/9/10 未开工。
- 下一步：用户按 UI 实测清单验证 4 项修正 → 反馈；无问题后进入阶段 8 其余 3 项确认。
