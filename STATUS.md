...
- 当前状态：v2.0 阶段 3/4/7/8 联测反馈第 2 轮修正已完成（①读取拍照位姿/读取戳点位姿
  按钮并入主界面底部 ActionButtons 同一行、与预览/拍照/计算样式字体一致；拍照时自动
  读取勾选迁回工具面板机器人通信页；②3D 复位按钮改为悬浮原生无边框胶囊，重叠于场景
  左下角，不再有独立工具栏条）。AICode 与 D 工作区构建+单测全绿，待用户再实测；
  阶段 5/6/9/10 未开工。
- 下一步：用户按 UI 清单实测第 2 轮两处修正 → 反馈；无问题后进入阶段 8 其余 3 项确认。
- 正在进行的文件：无（等待实测）；反馈后按需改 src/app/MainWindow、src/ui。
- 卡点 / 风险：3D 复位按钮为原生子窗口（WA_TranslucentBackground），在目标机需确认
  胶囊四角透明与可点击正常；阶段 8 姿态引导/质检/板位姿可视化规格未确认；
  机器人/Modbus 真机未到位；阶段 4 外参语义待真实线扫数据验证。
...
- [ ] 用户 UI 实测第 2 轮：读取按钮并入 ActionButtons 行、3D 复位按钮悬浮胶囊样式/可点击
- [ ] 用户 UI 实测：4 项修正（机器人读取按钮显示/样式、窗口居中、缩放顺滑、视窗去标题）
