...
const db = new Low(adapter, { users: [], customers: [], tickets: [], worklogs: [] });
...
async function initDb() {
  ...
  if (!db.data) db.data = { users: [], customers: [], tickets: [], worklogs: [] };
}
...
const dbApi = {
  ...
  async createCustomer(c) { await initDb(); if (!c.id) { c.id = generateId('cust'); c.createdAt = new Date().toISOString(); } db.data.customers.push(c); await db.write(); return c; },
  ...
  async getWorklogs() { await initDb(); return db.data.worklogs || []; },
  async getWorklogById(id) { await initDb(); return (db.data.worklogs || []).find(w => w.id === id); },
  async createWorklog(w) { await initDb(); db.data.worklogs = db.data.worklogs || []; db.data.worklogs.push(w); await db.write(); return w; },
  async updateWorklog(id, fields) {
    await initDb();
    const idx = (db.data.worklogs || []).findIndex(w => w.id === id);
    if (idx >= 0) { Object.assign(db.data.worklogs[idx], fields); await db.write(); }
  },
  async deleteWorklog(id) {
    await initDb();
    db.data.worklogs = (db.data.worklogs || []).filter(w => w.id !== id);
    await db.write();
  },
