...
BoxCropResult cropByBoxes(const ObjectList& input,
    ...
    for (std::size_t i = 0; i < input.objects.size(); ++i) {
        ...
        // Every input frame is processed with every box (F x M alignment,
        // PROJECT §8.3.3); the properties-panel selection only drives the
        // 3D view / ROI baseline, not which frames get cropped.
