...
class WorkLogPro(QMainWindow):
        ...
        if self.is_table_selection and self.current_selected_row >= 0:
            ...
            if is_edit_mode:
                ...
                if status_changed:
                    ...
                    solution = data.get('solution', '')[:50] + '...' if len(data.get('solution', '')) > 50 else data.get('solution', '')
                    table_data = [
                        new_id,
                        data.get('date', ''),
                        data.get('customer', ''),
            ...
            try:
                ...
                for row in range(self.table.rowCount()):
                    record = {
                        ...
                        '日期': self.table.item(row, 1).text(),
                        '客户名称': self.table.item(row, 2).text(),
                        '相机型号': self.table.item(row, 3).text(),
                        '客户问题': self.table.item(row, 4).text(),
                        '解决方法': self.table.item(row, 5).text(),
                        '问题类型': self.table.item(row, 6).text(),
                        '问题进度': self.table.item(row, 7).text()
                    }
                    data.append(record)
    ...
    def on_header_clicked(self, logical_index):
        """表头点击事件处理，实现排序功能"""
        # 只对客户名称(3)和相机型号(4)列进行排序
        if logical_index not in [3, 4]:
            return
        
        # 获取列名称
        column_name = "客户名称" if logical_index == 3 else "相机型号"
        
        if self.sort_column == logical_index:
            # 再次点击同一列，恢复原始排列方式
            self.add_log(f"恢复 {column_name} 原始排列方式", "操作")
            self.sort_column = -1  # 重置排序列
            # 重新加载数据
            self.load_data_from_excel()
        else:
            # 点击列，按升序排序
            ...
            # 按升序排序数据
            rows_data.sort(key=lambda x: x[logical_index])
    ...
    def add_log(self, message: str, level: str = "信息"):
        ...
        try:
            ...
            # 日志文件写入失败不影响程序运行，但会在界面日志中显示
            error_entry = f'<span style="color:#718096">[{timestamp}]</span> <span style="color:#e53e3e;font-weight:bold">[错误]</span> <span style="color:#2d3748">日志文件写入失败: {str(e)}</span>'
            self.log_text.append(error_entry)
