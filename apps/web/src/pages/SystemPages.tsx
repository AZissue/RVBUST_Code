...
export function SettingsPage() {
  ...
  return <div className="page-stack"><header className="page-header"><div><span className="eyebrow">SYSTEM</span><h1>系统设置</h1><p>这里只保存非敏感工作区配置；密钥和密码不会通过此页面存储。</p></div></header><section className="panel settings-panel"><div className="section-heading"><div><h2><Settings size={18} />工作区</h2><p>系统显示名称和地区设置</p></div></div><form onSubmit={async (event) => { event.preventDefault(); const form = new FormData(event.currentTarget); await api('/system/settings', { method: 'PUT', body: JSON.stringify({ key: 'workspace', isPublic: true, value: { name: form.get('name'), locale: form.get('locale') } }) }); setSaved('设置已保存'); await remote.refresh() }}><label>工作区名称<input name="name" defaultValue={String(workspace?.value?.name ?? '技术支持系统 V2')} /></label><label>语言<select name="locale" defaultValue={String(workspace?.value?.locale ?? 'zh-CN')}><option value="zh-CN">简体中文</option></select></label><button className="button primary">保存设置</button>{saved && <span className="success-text">{saved}</span>}</form></section><section className="panel settings-panel"><div className="section-heading"><div><h2>工作分类</h2><p>工作事项和工作记录共用，可停用但不删除历史分类。</p></div><button className="button" onClick={() => setCreatingType(true)}><Plus size={15} />新增分类</button></div><div className="relation-list">{types.data?.map((type) => <div className="relation-row" key={type.id}><div><strong>{type.label}</strong><span className="mono">{type.code}</span></div><button className="button small" onClick={async () => { await api(`/work-types/${type.id}`, { method: 'PATCH', body: JSON.stringify({ isActive: !type.isActive }) }); await types.refresh() }}>{type.isActive ? '停用' : '启用'}</button></div>)}</div></section>{creatingType && <SimpleFormModal title="新增工作分类" fields={[["label", "分类名称", true], ["code", "分类编码（小写英文和短横线）", true], ["description", "说明"]]} onClose={() => setCreatingType(false)} onSubmit={async (payload) => { await api('/work-types', { method: 'POST', body: JSON.stringify(payload) }); setCreatingType(false); await types.refresh() }} />}</div>
}
