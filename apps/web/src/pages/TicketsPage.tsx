...
import { TICKET_CATEGORIES, ticketCategoryLabel, ticketCategoryLabels, ticketEventTypeLabel } from '../lib/labels'
import type { LoanStatus, Ticket, TicketEvent, TicketStatus } from '../types'
...
export function TicketDetailPage() {
  ...
  return <div className="page-stack">
    <header className="detail-header"><button className="icon-button" onClick={() => { const target = (location.state as { returnTo?: string } | null)?.returnTo; navigate(target && /^\/(tickets|my-work)(\?|$)/.test(target) ? target : '/tickets') }} title="返回"><ArrowLeft size={20} /></button><div><span className="mono eyebrow">{ticket.number}</span><h1>{ticket.title}</h1><div className="inline-meta"><StatusBadge status={ticket.status} /><PriorityBadge priority={ticket.priority} /><span>{ticket.organization.name}</span></div></div><div className="header-actions">{editable && !isDeleted && <button type="button" className="button" onClick={() => setEditing(true)}><Pencil size={15} />编辑工单</button>}{canOperate && !isDeleted ? <select aria-label="修改工单状态" className={`status-select status-${ticket.status.toLowerCase()}`} value={ticket.status} onChange={(event) => void changeStatus(event.target.value as TicketStatus)}>{states.map((item) => <option key={item} value={item}>{statusLabel(item)}</option>)}</select> : <StatusBadge status={ticket.status} />}</div></header>
    {ticket.description && <section className="ticket-description"><p>{ticket.description}</p></section>}
    ...
    {editing && <TicketBasicInfoModal ticket={ticket} onClose={() => setEditing(false)} onSaved={async () => { setEditing(false); await remote.refresh() }} />}
...
function TicketLinkageCard({ ticket, isDeleted, onCreated }: { ticket: Ticket; isDeleted: boolean; onCreated: (message: string) => Promise<void> }) {
  const [creating, setCreating] = useState<'loan' | 'repair' | ''>('')
  const loanOrders = ticket.loanOrders ?? []
  ...
  const create = async (kind: 'loan' | 'repair') => {
    ...
    try {
      if (kind === 'loan') {
        const result = await api<{ loan: { loanNo: string } }>(`/tickets/${ticket.id}/loan-requests`, { method: 'POST' })
        await onCreated(`借测单 ${result.loan.loanNo} 已创建并加入排队`)
      } else {
        const result = await api<{ repair: { repairNo: string } }>(`/tickets/${ticket.id}/repair-requests`, { method: 'POST' })
        await onCreated(`维修单 ${result.repair.repairNo} 已创建`)
      }
    } catch (reason) { window.alert(reason instanceof Error ? reason.message : '创建失败') } finally { setCreating('') }
  }
  ...
  return <section className="panel linkage-panel">
    ...
    {(loanOrders.length > 0 || repairOrders.length > 0) && <div className="linkage-list">
      ...
      {repairOrders.map(repair => <div key={repair.id} className={`linkage-item${repair.status === 'CLOSED' ? ' is-done' : ''}`}>
        <header><Link className="mono" to={`/repairs/${repair.id}`}>{repair.repairNo}</Link><span className="muted">维修单</span><RepairStatusBadge status={repair.status} /></header>
      </div>)}
    </div>}
    {(showLoanButton || showRepairButton) && <div className="row-actions">
...
function TimelineEventBody({ event }: { event: TicketEvent }) {
  if (event.type === 'STATUS_CHANGE') return <StatusTransition content={event.content} />
  // 联动事件：正文保留服务端 content，单号渲染为跳转链接；metadata 缺失时退化为纯文本
  if ((event.type === 'LINK_CREATED' || event.type === 'LINK_UPDATE') && event.metadata?.linkType && event.metadata.linkId) {
    const { linkType, linkId, linkNo } = event.metadata
    const to = linkType === 'loan' ? `/loans/${linkId}` : `/repairs/${linkId}`
    return <>{event.content} <Link className="mono timeline-link-ref" to={to} title={linkType === 'loan' ? '查看借测单' : '查看维修单'}>{linkNo || linkId}</Link></>
  }
  return <>{event.content}</>
}

const ACTIVE_LOAN_STATUSES: LoanStatus[] = ['QUEUED', 'ONGOING', 'OVERDUE']

function TicketBasicInfoModal({ ticket, onClose, onSaved }: { ticket: Ticket; onClose: () => void; onSaved: () => Promise<void> }) {
  const customers = useRemote(() => api<Array<{ id: string; name: string }>>('/customers'), [])
  const [error, setError] = useState('')
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await api(`/tickets/${ticket.id}`, { method: 'PATCH', body: JSON.stringify({ organizationId: value('organizationId'), category: value('category'), title: value('title'), description: value('description') }) })
      await onSaved()
    } catch (reason) { setError(reason instanceof Error ? reason.message : '保存失败') } finally { setBusy(false) }
  }
  return <Modal title="编辑工单信息" onClose={() => { if (!busy) onClose() }} wide><form className="form-grid" onSubmit={submit}>
    <label className="span-2">客户公司{customers.data ? <select name="organizationId" defaultValue={ticket.organization.id}>{customers.data.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select> : <span>加载中…</span>}</label>
    <label>问题分类<select name="category" defaultValue={ticket.category}>{TICKET_CATEGORIES.map((item) => <option key={item} value={item}>{ticketCategoryLabels[item]}</option>)}</select></label>
    <label className="span-2">问题标题<input name="title" required minLength={3} maxLength={240} defaultValue={ticket.title} /></label>
    <label className="span-2">问题描述<textarea name="description" required minLength={3} maxLength={20000} rows={5} defaultValue={ticket.description} /></label>
    {error && <div role="alert" className="form-error span-2">{error}</div>}
    <div className="form-actions span-2"><button type="button" className="button" disabled={busy} onClick={onClose}>取消</button><button className="button primary" disabled={busy || customers.loading || !customers.data}>{busy ? '正在保存' : '保存'}</button></div>
  </form></Modal>
}
