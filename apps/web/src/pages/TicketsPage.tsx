...
import { statusChangeLabel, ticketEventTypeLabel } from '../lib/labels'
...
export function TicketsPage({ mine = false }: { mine?: boolean }) {
  ...
  const canEditRow = (ticket: Ticket) => user?.role === 'admin' || user?.id === ticket.createdBy?.id || user?.id === ticket.assignee?.id
  const canTransferRow = (ticket: Ticket) => user?.role === 'admin' || user?.id === ticket.assignee?.id
  const changeStatus = async (ticket: Ticket, next: TicketStatus) => { try { await api(`/tickets/${ticket.id}/status`, { method: 'POST', body: JSON.stringify({ status: next }) }); await remote.refresh() } catch (reason) { window.alert(reason instanceof Error ? reason.message : '状态更新失败') } }
