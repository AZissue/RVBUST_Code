import { ArrowLeft, CheckCircle2, ClipboardList, Clock3, MessageSquare, Plus, RotateCcw, Search, Send, Trash2, UserPlus, X } from 'lucide-react'
...
import { ticketCategoryLabel, ticketAssistStatusLabel, ticketEventTypeLabel } from '../lib/labels'
...
export function TicketsPage({ mine = false }: { mine?: boolean }) {
  ...
  if (!mine && mineOnly) query.set('mine', '1')
  if (debouncedSearch.trim()) query.set('search', debouncedSearch.trim())
  if (serverStatus) query.set('status', serverStatus)
  query.set('page', String(page))
  const remote = useRemote(() => api<PagedTickets>(`/tickets?${query.toString()}`), [mine, mineOnly, debouncedSearch, status, page], true)
  ...
  const metricCount = (value: string) => value === '' ? total : value === 'WAITING' ? (byStatus.WAITING_CUSTOMER ?? 0) + (byStatus.WAITING_RND ?? 0) : value === 'DONE' ? (byStatus.RESOLVED ?? 0) + (byStatus.CLOSED ?? 0) : (byStatus[value as TicketStatus] ?? 0)
  ...
  const removeTicket = async (ticket: Ticket) => { if (!window.confirm(`确认将工单 ${ticket.number}「${ticket.title}」移入回收站？`)) return; const reason = window.prompt('删除原因（可选）：') ?? ''; try { await api(`/tickets/${ticket.id}`, { method: 'DELETE', body: JSON.stringify({ reason }) }); await remote.refresh() } catch (reason) { window.alert(reason instanceof Error ? reason.message : '删除失败') } }
  ...
  return <div className="page-stack ticket-queue">
    <header className="page-header"><div><span className="eyebrow">{mine ? 'MY WORK' : 'TICKET QUEUE'}</span><h1>{mine ? '我的工作' : '工单管理'}</h1></div><div className="header-actions"><button className="button" onClick={() => navigate('/tickets/recycle-bin')}><Trash2 size={16} />回收站</button><button className="button primary" onClick={() => setCreating(true)}><Plus size={16} />新建工单</button></div></header>
    ...
    <section className="toolbar"><div className="searchbox"><Search size={16} /><input placeholder="搜索编号、问题、客户或设备" value={search} onChange={(event) => setSearch(event.target.value)} /></div><select aria-label="工单状态筛选" value={status} onChange={(event) => setStatus(event.target.value)}><option value="">全部状态</option>{states.map((item) => <option key={item} value={item}>{statusLabel(item)}</option>)}<option value="WAITING">等待反馈</option><option value="DONE">已完成</option></select>{!mine && <label className="mine-toggle"><input type="checkbox" checked={mineOnly} onChange={(event) => setMineOnly(event.target.checked)} />只看我的</label>}<span className="result-count">{search !== debouncedSearch ? '搜索中…' : `${total} 张工单`}</span></section>
...
export function TicketDetailPage() {
  ...
  const removeTicket = async () => { if (!window.confirm(`确认将工单 ${ticket.number}「${ticket.title}」移入回收站？`)) return; const reason = window.prompt('删除原因（可选）：') ?? ''; try { await api(`/tickets/${id}`, { method: 'DELETE', body: JSON.stringify({ reason }) }); navigate('/tickets/recycle-bin', { replace: true }) } catch (reason) { setError(reason instanceof Error ? reason.message : '删除失败') } }
  ...
  const inviteAssist = async () => { if (!inviteUserIds.length) { setError('请选择协助对象'); return } try { await api(`/tickets/${id}/assist-requests`, { method: 'POST', body: JSON.stringify({ targetUserIds: inviteUserIds, message: inviteMessage.trim() || undefined }) }); setInviteOpen(false); setInviteUserIds([]); setInviteMessage(''); await remote.refresh() } catch (reason) { setError(reason instanceof Error ? reason.message : '邀请失败') } }
  const acceptAssist = async (requestId: string) => { try { await api(`/tickets/${id}/assist-requests/${requestId}/accept`, { method: 'POST' }); await remote.refresh() } catch (reason) { setError(reason instanceof Error ? reason.message : '接受失败') } }
  const rejectAssist = async (requestId: string) => { const reason = window.prompt('请填写驳回意见：'); if (reason === null) return; if (!reason.trim()) { setError('驳回意见不能为空'); return } try { await api(`/tickets/${id}/assist-requests/${requestId}/reject`, { method: 'POST', body: JSON.stringify({ reason: reason.trim() }) }); await remote.refresh() } catch (reason) { setError(reason instanceof Error ? reason.message : '驳回失败') } }
  const cancelAssist = async (requestId: string) => { try { await api(`/tickets/${id}/assist-requests/${requestId}/cancel`, { method: 'POST' }); await remote.refresh() } catch (reason) { setError(reason instanceof Error ? reason.message : '撤销失败') } }
  return <div className="page-stack">
    ...
    <div className="ticket-layout"><section className="panel"><div className="section-heading"><div><h2>处理时间线</h2><p>内部跟进记录，仅团队可见</p></div></div><div className="timeline">{ticket.events?.map((event) => <article key={event.id}><div className="timeline-dot" /><header><strong>{event.author.name}</strong><span className={`visibility ${event.visibility.toLowerCase()}`}>{event.visibility === 'INTERNAL' ? '内部' : '客户可见'}</span><time>{formatDate(event.createdAt)}</time></header><p>{event.type === 'STATUS_CHANGE' ? <StatusTransition content={event.content} /> : event.content}</p><small>{ticketEventTypeLabel(event.type)}</small></article>)}</div>
      ...
      <aside className="detail-aside"><section><h2>工单上下文</h2><dl><dt>客户</dt><dd><Link to={`/customers/${ticket.organization.id}`}>{ticket.organization.name}</Link></dd><dt>联系人</dt><dd>{ticket.contact?.name ?? '-'}</dd><dt>创建人</dt><dd>{ticket.createdBy?.name ?? '-'}</dd><dt>创建时间</dt><dd>{formatDate(ticket.createdAt)}</dd><dt>负责人</dt><dd>{ticket.assignee?.name ?? '未分配'}</dd><dt>计划完成</dt><dd>{formatDate(ticket.plannedAt)}</dd><dt>分类</dt><dd>{ticketCategoryLabel(ticket.category)}</dd></dl></section><section><h2>协助</h2><div className="assist-list">{(ticket.assistRequests ?? []).map((request) => <div className="assist-item" key={request.id}><header><strong>{request.targetUser.name}</strong><span className={`badge assist-${request.status.toLowerCase()}`}>{ticketAssistStatusLabel(request.status)}</span></header><small>{request.requester.name} 邀请 · {formatDate(request.createdAt)}</small>{request.message && <p>{request.message}</p>}{request.rejectReason && <p className="assist-reason">驳回意见：{request.rejectReason}</p>}{request.status === 'PENDING' && request.targetUserId === user?.id && <div className="row-actions"><button className="button small" onClick={() => void acceptAssist(request.id)}>接受</button><button className="button small danger" onClick={() => void rejectAssist(request.id)}>驳回</button></div>}{request.status === 'PENDING' && (request.requesterId === user?.id || user?.role === 'admin') && <div className="row-actions"><button className="button small" onClick={() => void cancelAssist(request.id)}>撤销</button></div>}</div>)}</div>{!ticket.assistRequests?.length && <p className="muted">暂无协助邀请</p>}{editable && !isDeleted && <div className="row-actions"><button className="button" onClick={() => setInviteOpen((previous) => !previous)}><UserPlus size={15} />邀请协助</button></div>}{inviteOpen && editable && !isDeleted && <div className="invite-box">{assignable.data?.map((item) => <label key={item.id} className="checkbox-row"><input type="checkbox" checked={inviteUserIds.includes(item.id)} onChange={() => toggleInvite(item.id)} />{item.name}</label>) ?? <span>加载中…</span>}<input value={inviteMessage} onChange={(event) => setInviteMessage(event.target.value)} placeholder="协助说明（可选）" maxLength={2000} /><div className="row-actions"><button className="button primary small" onClick={() => void inviteAssist()}>发出邀请</button><button className="button small" onClick={() => setInviteOpen(false)}>取消</button></div></div>}</section><section className="recycle-actions">{!isDeleted && editable && <button className="button danger" onClick={() => void removeTicket()}><Trash2 size={15} />移入回收站</button>}{isDeleted && <button className="button" onClick={() => void restoreTicket()}><RotateCcw size={16} />恢复工单</button>}</section><section><h2>设备环境</h2><dl><dt>相机型号</dt><dd>{ticket.cameraModel || '-'}</dd><dt>SN</dt><dd className="mono">{ticket.serialNumber || '-'}</dd><dt>SDK</dt><dd>{ticket.sdkVersion || '-'}</dd><dt>系统环境</dt><dd>{ticket.systemEnvironment || '-'}</dd></dl></section><section><h2>问题描述</h2><p>{ticket.description}</p></section></aside>
    </div>
  </div>
}
