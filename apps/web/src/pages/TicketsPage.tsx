...
export function TicketDetailPage() {
  ...
  return <div className="page-stack">
    <section className="toolbar"><label>负责人<select aria-label="工单负责人" disabled={!editable || saving || !users.data} value={ticket.assignee?.id ?? ''} onChange={(e) => void assign(e.target.value)}><option disabled value="">未分配</option>{users.data?.map((u) => <option value={u.id} key={u.id}>{u.name}</option>)}</select></label></section>
