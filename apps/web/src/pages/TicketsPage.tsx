...
export function TicketsPage({ mine = false }: { mine?: boolean }) {
  ...
  const remote = useRemote(() => api<PagedTickets>(`/tickets?${query.toString()}`), [mine, debouncedSearch, status, page], true)
  ...
  return <div className="page-stack ticket-queue">
    ...
    <section className="toolbar"><div className="searchbox"><Search size={16} /><input placeholder="搜索编号、问题、客户或设备" value={search} onChange={(event) => setSearch(event.target.value)} /></div><select aria-label="工单状态筛选" value={status} onChange={(event) => setStatus(event.target.value)}><option value="">全部状态</option>{states.map((item) => <option key={item} value={item}>{statusLabel(item)}</option>)}<option value="WAITING">等待反馈</option><option value="DONE">已完成</option></select><span className="result-count">{search !== debouncedSearch ? '搜索中…' : `${total} 张工单`}</span></section>
