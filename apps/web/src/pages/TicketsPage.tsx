...
export function TicketDetailPage() {
  ...
  return <div className="page-stack">
    <header className="detail-header"><button className="icon-button" onClick={() => { const target = (location.state as { returnTo?: string } | null)?.returnTo; navigate(target && /^\/(tickets|my-work)(\?|$)/.test(target) ? target : '/tickets') }} title="返回"><ArrowLeft size={20} /></button><div><span className="mono eyebrow">{ticket.number}</span><h1>{ticket.title}</h1><div className="inline-meta"><StatusBadge status={ticket.status} /><PriorityBadge priority={ticket.priority} /><span>{ticket.organization.name}</span></div></div>{canOperate ? <select className="status-select" value={ticket.status} onChange={(event) => void changeStatus(event.target.value as TicketStatus)}>{states.map((item) => <option key={item} value={item}>{statusLabel(item)}</option>)}</select> : <StatusBadge status={ticket.status} />}</header>
