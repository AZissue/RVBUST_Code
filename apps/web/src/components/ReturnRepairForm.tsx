...
export function ReturnRepairForm({ repair, onClose, onSaved }: { repair?: RepairOrder; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const [data, setData] = useState<ReturnForm>(repair?.returnForm ?? {
    ...
    reporterName: repair?.contact?.name ?? '', reporterPhone: '', serialNumber: repair?.device.serialNumber ?? '',
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      let id = savedId
      if (!id) {
        const body = repair ? { returnForm: data, symptom } : { organizationId, deviceId: deviceId || undefined, serialNumber: data.serialNumber, symptom, returnForm: data }
    ...
    <form onSubmit={submit}>
      ...
      <fieldset disabled={busy || Boolean(savedId)} className="return-form-fields form-grid">
        {!repair && <><label>关联客户<select required value={organizationId} onChange={e => { setOrganizationId(e.target.value); setDeviceId(''); field('companyName', customers.data?.find(c => c.id === e.target.value)?.name ?? ''); field('serialNumber', '') }}><option value="">选择客户</option>{customers.data?.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
          <label>关联设备<select value={deviceId} onChange={e => { setDeviceId(e.target.value); field('serialNumber', devices.find(d => d.id === e.target.value)?.serialNumber ?? '') }}><option value="">按下方 SN 匹配台账设备</option>{devices.map(d => <option key={d.id} value={d.id}>{d.name} · {d.serialNumber}</option>)}</select></label></>}
        ...
        <label>报修设备 SN<input required maxLength={120} value={data.serialNumber} onChange={e => field('serialNumber', e.target.value)} /></label>
