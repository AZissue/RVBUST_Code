...
function QuickTicketConfirm({ parsed, canCreateCustomer, onClose, onSaved }: { parsed: Parsed; canCreateCustomer: boolean; onClose: () => void; onSaved: (ticket: Ticket, updated: boolean) => void }) {
  ...
  return <Modal title="解析结果确认" onClose={() => { if (!busy) onClose() }} wide><div className="quick-confirm">
    ...
    <fieldset disabled={busy} className="form-grid">
      ...
      <label className="span-2">问题分类<select aria-label="确认问题分类" value={category} onChange={(e) => setCategory(e.target.value as TicketCategory)}>{Object.entries(ticketCategoryLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><small>按关键词初步识别，可修改</small></label>
