...
describe('Quick tickets and single-source personal work', () => {
  ...
  it('creates a real ticket exactly once and scopes my work by assignee only', async () => {
    const dto = { source: 'AFTER_SALES_INCIDENT', category: '硬件故障', organizationId: orgId, deviceId, assigneeId: supportId, cameraModel: 'M2600', title: 'M2600连接超时', description: 'M2600连接超时', rawText: '浙江智享 M2600连接超时 李四 紧急', priority: 'URGENT', requestKey: randomUUID() };
  ...
  it('updates personal and dashboard status from the same ticket', async () => {
    ...
    const overdue = (await admin.post('/api/tickets').send({ source: 'OTHER', category: '其他', organizationId: orgId, assigneeId: adminId, title: '逾期计划验证', description: '逾期计划验证', plannedAt: new Date(Date.now() - 86400000).toISOString() }).expect(201)).body;
  ...
  it('rejects missing customer, invalid assignee, cross-customer device and blank descriptions', async () => {
    const dto = { source: 'OTHER', category: '其他', organizationId: orgId, title: 'M2600无点云', description: 'M2600无点云' };
