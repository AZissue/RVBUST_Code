...
describe('AccessPolicyService', () => {
  it('lets all internal roles see every ticket', () => {
    const employee = { id: 'u2', username: 'employee', name: '员工', email: null, role: 'employee', permissions: [] };
  ...
  it('limits employee work items to ownership or collaboration', () => {
    expect(service.workItemWhere({ id: 'u2', username: 'employee', name: '员工', email: null, role: 'employee', permissions: [] })).toEqual({ OR: [{ ownerId: 'u2' }, { collaborators: { some: { userId: 'u2' } } }] });
  });

  it('limits employee work logs to authored facts', () => {
    expect(service.worklogWhere({ id: 'u2', username: 'employee', name: '员工', email: null, role: 'employee', permissions: [] })).toEqual({ authorId: 'u2' });
  });
});
