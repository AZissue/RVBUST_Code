...
export class WorkitemsController {
  ...
  @Get(':id') get(@CurrentUser() user: AuthUser, @Param('id', ParseUUIDPipe) id: string) { return this.workitems.get(user, id); }
  @Post() create(@CurrentUser() user: AuthUser, @Body() dto: CreateWorkItemDto) { return this.workitems.create(user, dto); }
  @Patch(':id') update(@CurrentUser() user: AuthUser, @Param('id', ParseUUIDPipe) id: string, @Body() dto: UpdateWorkItemDto) { return this.workitems.update(user, id, dto); }
  @Delete(':id') remove(@CurrentUser() user: AuthUser, @Param('id', ParseUUIDPipe) id: string) { return this.workitems.remove(user, id); }
}
